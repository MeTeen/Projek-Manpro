// Crm-backend/src/seeders/YYYYMMDDHHMMSS-initial-sample-data.js
// Ganti YYYYMMDDHHMMSS dengan timestamp aktual file Anda
'use strict';
...
module.exports = {
  async up (queryInterface, Sequelize) {
    const now = new Date(); // Titik waktu saat ini
    let seededAdminIds = [];
    ...
    // --- 1. Admins ---
    // ... (kode seeding admin Anda, sudah benar dari sebelumnya) ...
    ...
    const existingAdmins = await queryInterface.sequelize.query(
      ...
      { replacements: { adminEmails }, type: queryInterface.sequelize.QueryTypes.SELECT }
    );
    ...
    if (newAdminsToInsert.length > 0) {
      await queryInterface.bulkInsert('admins', newAdminsToInsert, { returning: ['id'] });
      ...
      const newlyInsertedAdmins = await queryInterface.sequelize.query(
        ...
        { replacements: { newAdminEmails: newAdminsToInsert.map(a=>a.email) }, type: queryInterface.sequelize.QueryTypes.SELECT }
      );
    ...
    // --- 2. Customers ---
    // ... (kode seeding customer Anda, sudah benar dari sebelumnya, pastikan nama kolom DB sesuai) ...
    ...
    const existingDbCustomers = await queryInterface.sequelize.query(
      ...
      { replacements: { customerEmails }, type: queryInterface.sequelize.QueryTypes.SELECT }
    );
    ...
    const newCustomersToInsert = customersToSeed
      ...
      .map(c => ({ 
        ...
        address: c.address, city: c.city, state: c.state, zip_code: c.zipCode,
        avatar_url: c.avatarUrl, total_spent: c.totalSpent, purchase_count: c.purchaseCount,
    ...
    if (newCustomersToInsert.length > 0) {
      ...
      const newlyInsertedCustomers = await queryInterface.sequelize.query(
        ...
        { replacements: { newCustomerEmails: newCustomersToInsert.map(c=>c.email) }, type: queryInterface.sequelize.QueryTypes.SELECT }
      );
    ...
    // --- 3. Products ---
    // ... (kode seeding product Anda, sudah benar dari sebelumnya) ...
    ...
    const existingDbProducts = await queryInterface.sequelize.query(
      ...
      { replacements: { productNames }, type: queryInterface.sequelize.QueryTypes.SELECT }
    );
    ...
    if (newProductsToInsert.length > 0) {
      ...
      const newlyInsertedProducts = await queryInterface.sequelize.query(
        ...
        { replacements: { newProductNames: newProductsToInsert.map(p=>p.name) }, type: queryInterface.sequelize.QueryTypes.SELECT }
      );
    ...
    // --- 4. Promos ---
    // ... (kode seeding promo Anda, sudah benar dari sebelumnya, pastikan created_by diisi) ...
    console.log('Seeding Promos...');
    const startDatePromo = new Date(now); // Buat instance baru agar tidak mengubah 'now'
    startDatePromo.setMonth(startDatePromo.getMonth() - 2); // Promo dimulai 2 bulan lalu
    const endDatePromo = new Date(startDatePromo);
    endDatePromo.setDate(startDatePromo.getDate() + 60); // Promo berlaku 60 hari dari start_date nya
    ...
    const existingDbPromos = await queryInterface.sequelize.query(
      ...
      { replacements: { promoNames }, type: queryInterface.sequelize.QueryTypes.SELECT }
    );
    ...
    if (newPromosToInsert.length > 0) {
      ...
      const newlyInsertedPromos = await queryInterface.sequelize.query(
        ...
        { replacements: { newPromoNames: newPromosToInsert.map(p=>p.name) }, type: queryInterface.sequelize.QueryTypes.SELECT }
      );
    ...
    // --- 5. Tasks ---
    // ... (kode seeding task Anda, sudah benar dari sebelumnya) ...
    ...
    const existingDbTasks = await queryInterface.sequelize.query(
      ...
      { replacements: { taskContents }, type: queryInterface.sequelize.QueryTypes.SELECT }
    );
    ...
    const newTasksToInsert = tasksToSeed
        ...
        .map(t => ({ 
            date: t.date,
            content: t.content,
            isCompleted: t.isCompleted, // Pastikan nama kolom di DB (isCompleted atau isCompleted)
            createdAt: now,
            updatedAt: now
         }));
    ...
    // Transaksi 1: Budi beli Meja (2 bulan lalu)
    if (custBudiId && prodMejaId) {
      const purchaseDate = getDateMonthsAgo(2);
      purchasesData.push({
        customer_id: custBudiId, product_id: prodMejaId, quantity: 1,
        price: 1250000.00, purchase_date: purchaseDate, promo_id: null, discount_amount: 0.00,
        created_at: purchaseDate, updated_at: purchaseDate,
      });
    }

    // Transaksi 2: Ani beli Kursi, pakai promo Diskon 12% (1 bulan 15 hari lalu)
    if (custAniId && prodKursiId && promoDiskon12Id) {
      const hargaKursi = 950000.00;
      const purchaseDate = getDateDaysAgo(45); // ~1.5 bulan lalu
      purchasesData.push({
        customer_id: custAniId, product_id: prodKursiId, quantity: 1,
        price: hargaKursi, purchase_date: purchaseDate, promo_id: promoDiskon12Id,
        discount_amount: hargaKursi * 0.12,
        created_at: purchaseDate, updated_at: purchaseDate,
      });
    }
    
    // Transaksi 3: Citra beli 2 Lampu, pakai promo Potongan Rp 30.000 (20 hari lalu)
    if (custCitraId && prodLampuId && promoPotongan30Id) {
      const purchaseDate = getDateDaysAgo(20);
      purchasesData.push({
        customer_id: custCitraId, product_id: prodLampuId, quantity: 2,
        price: 175000.00, purchase_date: purchaseDate, promo_id: promoPotongan30Id,
        discount_amount: 30000.00,
        created_at: purchaseDate, updated_at: purchaseDate,
      });
    }
    
    // Transaksi 4: Budi beli Kursi lagi (10 hari lalu)
    if (custBudiId && prodKursiId) {
        const purchaseDate = getDateDaysAgo(10);
        purchasesData.push({
            customer_id: custBudiId, product_id: prodKursiId, quantity: 1,
            price: 950000.00, purchase_date: purchaseDate, promo_id: null, discount_amount: 0.00,
            created_at: purchaseDate, updated_at: purchaseDate,
        });
    }
    
    // Transaksi 5: Ani beli Lampu (5 hari lalu)
    if (custAniId && prodLampuId) {
        const purchaseDate = getDateDaysAgo(5);
        purchasesData.push({
            customer_id: custAniId, product_id: prodLampuId, quantity: 1,
            price: 175000.00, purchase_date: purchaseDate, promo_id: null, discount_amount: 0.00,
            created_at: purchaseDate, updated_at: purchaseDate,
        });
    }
    
    // Transaksi 6: Citra beli Meja (Hari ini)
    if (custCitraId && prodMejaId) {
        const purchaseDate = now; // atau getDateDaysAgo(0)
        purchasesData.push({
            customer_id: custCitraId, product_id: prodMejaId, quantity: 1,
            price: 1250000.00, purchase_date: purchaseDate, promo_id: null, discount_amount: 0.00,
            created_at: purchaseDate, updated_at: purchaseDate,
        });
    }
    ...
    if (purchasesData.length > 0) {
      ...
      await queryInterface.bulkDelete('customer_products', null, {});
    ...
    // --- 7. Update Customer total_spent dan purchase_count ---
    // ... (kode update customer totals Anda, sudah benar dari sebelumnya) ...
    ...
    for (const customer of allCustomersForUpdate) {
      ...
      await queryInterface.bulkUpdate('customers', {
        ...
        updated_at: now // Gunakan nama kolom db yang benar
      }, { id: customerId });
    }
  ...
  async down (queryInterface, Sequelize) {
    // ... (fungsi down Anda, sudah benar dari sebelumnya)
