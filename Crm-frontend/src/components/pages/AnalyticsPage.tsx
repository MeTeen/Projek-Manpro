...
const AnalyticsPage: React.FC = () => {
  ...
  const salesTrendLines = [
      { key: 'pendapatan', color: '#8884d8', name: 'Pendapatan (Rp)'},
      { key: 'transaksi', color: '#82ca9d', name: 'Jumlah Transaksi'}
  ];
          ...
          {salesTrendData.length > 0 ? (
            <ReusableLineChart
              data={salesTrendData.map(item => ({
                ...item,
                value: typeof item.pendapatan === 'number' ? item.pendapatan : 0 // or another default, adjust as needed
              }))}
              xAxisKey="name"
              lines={salesTrendLines}
              title="Tren Pendapatan & Transaksi"
            />
          ) : !loading && <p>Data tren penjualan tidak tersedia.</p>}
